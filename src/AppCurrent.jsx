...
import { Fish, MapPin, Trash2, Navigation, Lock, AlertTriangle, Thermometer, Wind, Cloud, CloudRain, Sunrise, Sunset, Crosshair, Bell } from "lucide-react";
import { DayPicker } from "react-day-picker";
import "react-day-picker/dist/style.css";
// mapbox css loaded dynamically in MapTab

// ─── DATE PICKER ──────────────────────────────────────────────────────────────
function DatePickerInput({ value, onChange, minDate, maxDate, placeholder = "Select date..." }) {
...
function PinDetailPage({ pin, onBack, onDelete, onSharePin, onSave }) {
  ...
  const checkConditions = async () => {
    ...
    setWeather(null);
    try {
      const res = await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${pin.lat}&longitude=${pin.lng}&current=temperature_2m,windspeed_10m,winddirection_10m,precipitation,cloudcover,weathercode&daily=sunrise,sunset&temperature_unit=fahrenheit&wind_speed_unit=mph&timezone=auto&forecast_days=1`);
      const data = await res.json();
      const c = data.current;
      const windDeg = c.winddirection_10m;
      const dirs = ["N","NE","E","SE","S","SW","W","NW"];
      const windDir = dirs[Math.round(windDeg / 45) % 8];
      const codes = { 0:"Clear", 1:"Mostly Clear", 2:"Partly Cloudy", 3:"Overcast", 45:"Foggy", 48:"Foggy", 51:"Light Drizzle", 53:"Drizzle", 55:"Heavy Drizzle", 61:"Light Rain", 63:"Rain", 65:"Heavy Rain", 71:"Light Snow", 73:"Snow", 75:"Heavy Snow", 80:"Rain Showers", 81:"Rain Showers", 82:"Heavy Showers", 95:"Thunderstorm", 96:"Thunderstorm", 99:"Thunderstorm" };
      const condition = codes[c.weathercode] || "Unknown";
      ...
      setWeather({ temp: Math.round(c.temperature_2m), windSpeed: Math.round(c.windspeed_10m), windDir, precip: c.precipitation, cloudcover: c.cloudcover, condition, sunrise, sunset });
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 1020, background: "#070e07", display: "flex", flexDirection: "column", fontFamily: "var(--font-body)" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "16px 18px", borderBottom: "1px solid var(--border)", background: "rgba(8,15,8,0.98)" }}>
        <button onClick={onBack} className="btn-ghost" style={{ padding: "6px 12px", fontSize: 13 }}>← Back</button>
        <span style={{ color: "var(--text)", fontWeight: 700, fontSize: 16 }}>{pin.spot_type || "Spot"} Detail</span>
        <button onClick={handleDelete} disabled={deleting} style={{ marginLeft: "auto", background: "none", border: "none", color: "rgba(255,100,100,0.75)", fontSize: 13, cursor: "pointer", fontFamily: "var(--font-body)", padding: "6px 10px" }}>
          {deleting ? "..." : <span style={{ display: "flex", alignItems: "center", gap: 5 }}><Trash2 size={13} />Delete</span>}
        </button>
      </div>
      <div style={{ flex: 1, overflowY: "auto", padding: "20px 18px", display: "flex", flexDirection: "column", gap: 16 }}>

        {/* Conditions card */}
        <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", padding: "14px 16px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: weather ? 12 : 0 }}>
            <div style={{ color: "var(--text2)", fontSize: 13, fontWeight: 600, display:"flex", alignItems:"center", gap:6 }}><Cloud size={13} />Conditions at this spot</div>
            <button onClick={checkConditions} disabled={weatherLoading} style={{ background: "linear-gradient(135deg,#78b450,#4a8a2a)", border: "none", borderRadius: 10, color: "white", fontSize: 12, fontWeight: 700, padding: "6px 12px", cursor: "pointer", fontFamily: "var(--font-body)", opacity: weatherLoading ? 0.6 : 1 }}>
              {weatherLoading ? "Loading..." : weather ? "Refresh" : "Check Now"}
            </button>
          </div>
          {weather && !weather.error && (
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                <div style={{ background: "rgba(255,255,255,0.05)", borderRadius: 10, padding: "8px 12px", fontSize: 13, color: "var(--text)", display: "flex", alignItems: "center", gap: 5 }}><Thermometer size={13} />{weather.temp}°F</div>
                <div style={{ background: "rgba(255,255,255,0.05)", borderRadius: 10, padding: "8px 12px", fontSize: 13, color: "var(--text)", display: "flex", alignItems: "center", gap: 5 }}><Wind size={13} />{weather.windSpeed} mph {weather.windDir}</div>
                <div style={{ background: "rgba(255,255,255,0.05)", borderRadius: 10, padding: "8px 12px", fontSize: 13, color: "var(--text)", display: "flex", alignItems: "center", gap: 5 }}><Cloud size={13} />{weather.cloudcover}%</div>
                <div style={{ background: "rgba(255,255,255,0.05)", borderRadius: 10, padding: "8px 12px", fontSize: 13, color: "var(--text)" }}>{weather.condition}</div>
                {weather.precip > 0 && <div style={{ background: "rgba(80,140,220,0.15)", borderRadius: 10, padding: "8px 12px", fontSize: 13, color: "#7ab0e0" }}><CloudRain size={13} />{weather.precip}mm precip</div>}
                {weather.sunrise && <div style={{ background: "rgba(255,255,255,0.05)", borderRadius: 10, padding: "8px 12px", fontSize: 13, color: "var(--text)", display: "flex", alignItems: "center", gap: 5 }}><Sunrise size={13} />{weather.sunrise}</div>}
                {weather.sunset && <div style={{ background: "rgba(255,255,255,0.05)", borderRadius: 10, padding: "8px 12px", fontSize: 13, color: "var(--text)", display: "flex", alignItems: "center", gap: 5 }}><Sunset size={13} />{weather.sunset}</div>}
              </div>
              {windMatch !== null && (
                <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 14px", borderRadius: 12, background: windMatch ? "rgba(120,180,80,0.12)" : "rgba(255,80,80,0.1)", border: `1px solid ${windMatch ? "rgba(120,180,80,0.3)" : "rgba(255,80,80,0.25)"}` }}>
                  <div style={{ width: 10, height: 10, borderRadius: "50%", background: windMatch ? "var(--green)" : "rgba(255,80,80,0.9)", flexShrink: 0 }} />
                  <span style={{ fontSize: 13, fontWeight: 700, color: windMatch ? "var(--green)" : "rgba(255,100,100,0.9)" }}>
                    {windMatch ? `Good wind for this spot — ${weather.windDir} wind is ideal` : `Bad wind — ${weather.windDir} wind, best is ${bestWind}`}
                  </span>
                </div>
              )}
            </div>
          )}
          {weather?.error && <div style={{ color: "rgba(255,100,100,0.7)", fontSize: 13, marginTop: 8 }}>Couldn't load conditions. Try again.</div>}
        </div>
